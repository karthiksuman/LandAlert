export const translations = {
  en: {
    systemTitle: "Landslide Early Warning & Disaster Management System",
    shortTitle: "LandAlert India",
    amISafe: "Am I Safe?",
    safeStatus: "Current Area Risk: LOW",
    unsafeStatus: "Current Area Risk: ELEVATED",
    criticalStatus: "CRITICAL DANGER: HIGH LANDSLIDE RISK",
    portals: {
      citizen: "Resident Portal",
      authority: "Authorities Portal",
      fieldOfficer: "Field Officer Portal",
      admin: "Admin Portal"
    },
    nav: {
      home: "Home",
      alerts: "Alerts",
      monitoring: "Risk Monitoring",
      safetyInsights: "Safety & Insights",
      report: "Report",
      more: "More",
      sos: "SOS EMERGENCY",
      fullScreen: "Expand Map",
      exitFullScreen: "Exit Full-Screen",
      layers: "GIS Layers",
      legend: "Risk Legend",
      recenter: "My Location"
    },
    moreSection: {
      title: "Safety Information & Settings",
      subtitle: "Disaster preparedness guides, emergency directory, and language configuration",
      tabGuide: "Survival Guide",
      tabHelplines: "Helpline Numbers",
      tabSettings: "Language & Settings",
      tabAbout: "About System",
      helplineSub: "Verified 24x7 National & State Emergency Hotlines",
      openSos: "Open Quick SOS",
      interfaceLanguage: "Interface Language",
      systemPermissions: "System Permissions",
      locationAccess: "Location Access",
      sirenNotify: "Siren Emergency Notifications",
      relaunchOnboarding: "Re-launch Resident Onboarding Setup",
      active: "ACTIVE",
      simulated: "SIMULATED REGIONAL",
      aboutTitle: "About LandAlert System",
      aboutDesc: "LandAlert is a government-grade disaster management and predictive early warning platform integrating IoT sensors, AI models, and real-time GIS spatial intelligence for North-Eastern India."
    },
    onboarding: {
      title: "Welcome to LandAlert",
      subtitle: "Government-Grade Landslide Early Warning & Disaster Management Platform",
      step1Title: "Select Your Language",
      step1Desc: "Choose your preferred language for safety alerts and instructions",
      step2Title: "Terms & Safety Agreement",
      step2Desc: "Please review official disaster data usage and resident guidelines",
      step2Terms: "By continuing, you agree that LandAlert provides AI-assisted predictive landslide risk assessments and emergency advisories. Always adhere to official NDRF, SDRF, and district administration evacuation directives.",
      step2Checkbox: "I accept the Terms and Conditions and Emergency Guidelines",
      step3Title: "Permissions Required",
      step3Desc: "Location and notification permissions are essential for localized life-saving warnings.",
      locationTitle: "Location Permission",
      locationDesc: "Used to pinpoint your position against real-time geological risk zones and calculate localized slope stability.",
      notifyTitle: "Emergency Notification Permission",
      notifyDesc: "Used to deliver immediate high-priority siren alerts for impending landslides and flash floods.",
      allowLocation: "Allow Location Access",
      allowNotify: "Allow Siren Notifications",
      granted: "Granted ✓",
      proceed: "ACCEPT & CONTINUE",
      tryAgain: "Try Again",
      continueWithoutLocation: "Continue with Regional Map",
      disclaimer: "Location access is required for personalized risk information. If denied, a general regional map of North-East India will be shown."
    },
    risk: {
      low: "Low / No Risk",
      moderate: "Moderate Risk",
      high: "High Risk",
      critical: "Critical Risk",
      rainfall: "24h Rainfall",
      soilMoisture: "Soil Moisture",
      groundMovement: "Ground Vibration",
      slope: "Slope Angle",
      terrain: "Geological Terrain",
      predictedProb: "Estimated Landslide Probability",
      whyRiskHigh: "Why is the risk elevated?",
      safetyPrecautions: "Safety Precautions"
    },
    report: {
      title: "Citizen Hazard Report",
      subtitle: "Submit real-time geo-tagged field observations directly to District Emergency Operations",
      hazardType: "Select Hazard Type",
      description: "Detailed Description of Hazard / Cracks / Debris",
      photoProof: "Attach Photo / Video Proof",
      captureGps: "Capture Current GPS",
      submitBtn: "SUBMIT REPORT TO AUTHORITIES",
      successTitle: "Report Submitted Successfully",
      successDesc: "Your incident report has been transmitted to the State Disaster Management Authority and assigned an emergency tracking ID.",
      reportId: "Report ID"
    },
    guide: {
      title: "Landslide Survival Guide",
      before: "Before a Landslide",
      during: "During a Landslide",
      after: "After a Landslide"
    }
  },
  hi: {
    systemTitle: "भूस्खलन पूर्व चेतावनी एवं आपदा प्रबंधन प्रणाली",
    shortTitle: "लैंड-अलर्ट इंडिया",
    amISafe: "क्या मैं सुरक्षित हूँ?",
    safeStatus: "वर्तमान क्षेत्र जोखिम: कम",
    unsafeStatus: "वर्तमान क्षेत्र जोखिम: बढ़ा हुआ",
    criticalStatus: "गंभीर चेतावनी: अत्यधिक भूस्खलन जोखिम",
    portals: {
      citizen: "निवासी पोर्टल",
      authority: "प्राधिकरण पोर्टल",
      fieldOfficer: "फील्ड अधिकारी पोर्टल",
      admin: "प्रशासक पोर्टल"
    },
    nav: {
      home: "होम",
      alerts: "चेतावनी",
      monitoring: "जोखिम निगरानी",
      safetyInsights: "सुरक्षा एवं अंतर्दृष्टि",
      report: "रिपोर्ट",
      more: "अधिक",
      sos: "आपातकालीन SOS",
      fullScreen: "नक्शा बड़ा करें",
      exitFullScreen: "सामान्य दृश्य",
      layers: "जीआईएस परतें",
      legend: "जोखिम संकेत",
      recenter: "मेरा स्थान"
    },
    moreSection: {
      title: "सुरक्षा जानकारी एवं सेटिंग्स",
      subtitle: "आपदा तैयारी गाइड, आपातकालीन निर्देशिका और भाषा विन्यास",
      tabGuide: "सुरक्षा गाइड",
      tabHelplines: "हेल्पलाइन नंबर",
      tabSettings: "भाषा एवं सेटिंग्स",
      tabAbout: "प्रणाली के बारे में",
      helplineSub: "सत्यापित 24x7 राष्ट्रीय एवं राज्य आपातकालीन हॉटलाइन",
      openSos: "त्वरित SOS खोलें",
      interfaceLanguage: "इंटरफ़ेस भाषा",
      systemPermissions: "सिस्टम अनुमतियाँ",
      locationAccess: "स्थान पहुँच (Location Access)",
      sirenNotify: "सायरन आपातकालीन सूचनाएं",
      relaunchOnboarding: "निवासी ऑनबोर्डिंग सेटअप पुनः शुरू करें",
      active: "सक्रिय",
      simulated: "सिम्युलेटेड क्षेत्रीय",
      aboutTitle: "लैंड-अलर्ट प्रणाली के बारे में",
      aboutDesc: "लैंड-अलर्ट पूर्वोत्तर भारत के लिए अत्याधुनिक सेंसर, एआई मॉडल और जीआईएस स्थानिक बुद्धिमत्ता को एकीकृत करने वाला सरकारी स्तर का आपदा प्रबंधन मंच है।"
    },
    onboarding: {
      title: "लैंड-अलर्ट में आपका स्वागत है",
      subtitle: "पूर्वोत्तर भारत के लिए एकीकृत एआई और सेंसर आधारित भूस्खलन पूर्व चेतावनी प्रणाली",
      step1Title: "अपनी भाषा चुनें",
      step1Desc: "सुरक्षा अलर्ट और निर्देशों के लिए अपनी पसंदीदा भाषा चुनें",
      step2Title: "नियम एवं सुरक्षा समझौता",
      step2Desc: "कृपया आधिकारिक आपदा डेटा उपयोग और निवासी दिशानिर्देशों की समीक्षा करें",
      step2Terms: "आगे बढ़कर, आप सहमत होते हैं कि लैंड-अलर्ट एआई-सहायता प्राप्त भूस्खलन जोखिम मूल्यांकन और सलाह प्रदान करता है। आधिकारिक एनडीआरएफ और एसडीआरएफ निर्देशों का पालन करें।",
      step2Checkbox: "मैं नियम, शर्तें और आपातकालीन दिशानिर्देश स्वीकार करता हूँ",
      step3Title: "आवश्यक अनुमतियाँ",
      step3Desc: "स्थान-विशिष्ट जीवन रक्षक चेतावनियों के लिए स्थान और अधिसूचना अनुमतियाँ आवश्यक हैं।",
      locationTitle: "स्थान अनुमति",
      locationDesc: "सटीक भूवैज्ञानिक जोखिम क्षेत्रों और ढलान स्थिरता की गणना के लिए उपयोग किया जाता है।",
      notifyTitle: "आपातकालीन चेतावनी अधिसूचना",
      notifyDesc: "आसन्न भूस्खलन और अचानक बाढ़ के लिए त्वरित सायरन अलर्ट भेजने हेतु उपयोग किया जाता है।",
      allowLocation: "स्थान अनुमति दें",
      allowNotify: "सूचनाएं अनुमति दें",
      granted: "स्वीकृत ✓",
      proceed: "स्वीकार करें और आगे बढ़ें",
      tryAgain: "पुनः प्रयास करें",
      continueWithoutLocation: "क्षेत्रीय नक्शे के साथ जारी रखें",
      disclaimer: "व्यक्तिगत जोखिम जानकारी के लिए स्थान का उपयोग आवश्यक है।"
    },
    risk: {
      low: "न्यूनतम / सुरक्षित",
      moderate: "मध्यम जोखिम",
      high: "उच्च जोखिम",
      critical: "गंभीर जोखिम",
      rainfall: "24 घंटे की वर्षा",
      soilMoisture: "मिट्टी की नमी",
      groundMovement: "भू-कंपन / विस्थापन",
      slope: "ढलान कोण",
      terrain: "भूगर्भीय भूभाग",
      predictedProb: "अनुमानित भूस्खलन संभावना",
      whyRiskHigh: "जोखिम क्यों अधिक है?",
      safetyPrecautions: "सुरक्षा सावधानियां"
    },
    report: {
      title: "नागरिक आपदा रिपोर्ट",
      subtitle: "जिला आपदा नियंत्रण कक्ष को भू-टैग की गई तस्वीरें और अवलोकन भेजें",
      hazardType: "खतरे का प्रकार चुनें",
      description: "खतरे, दरार या मलबे का विवरण",
      photoProof: "फोटो या वीडियो संलग्न करें",
      captureGps: "वर्तमान जीपीएस दर्ज करें",
      submitBtn: "प्राधिकारियों को रिपोर्ट भेजें",
      successTitle: "रिपोर्ट सफलतापूर्वक दर्ज की गई",
      successDesc: "आपकी रिपोर्ट राज्य आपदा प्रबंधन प्राधिकरण को भेज दी गई है।",
      reportId: "रिपोर्ट आईडी"
    },
    guide: {
      title: "भूस्खलन सुरक्षा गाइड",
      before: "भूस्खलन से पहले",
      during: "भूस्खलन के दौरान",
      after: "भूस्खलन के बाद"
    }
  },
  as: {
    systemTitle: "ভূমিস্খলন আগতীয়া সতৰ্কবাণী আৰু দুৰ্যোগ ব্যৱস্থাপনা প্ৰণালী",
    shortTitle: "লেণ্ড-এলাৰ্ট অসম",
    amISafe: "মই সুৰক্ষিতনে?",
    safeStatus: "বৰ্তমান অঞ্চলৰ আশংকা: নিম্ন",
    unsafeStatus: "বৰ্তমান অঞ্চলৰ আশংকা: মধ্যম",
    criticalStatus: "গুৰুতৰ বিপদ: উচ্চ ভূমিস্খলনৰ আশংকা",
    portals: {
      citizen: "বাসিন্দা পৰ্টেল",
      authority: "কৰ্তৃপক্ষ পৰ্টেল",
      fieldOfficer: "ক্ষেত্ৰ বিষয়া পৰ্টেল",
      admin: "প্ৰশাসক পৰ্টেল"
    },
    nav: {
      home: "গৃহ",
      alerts: "সতৰ্কবাণী",
      monitoring: "আশংকা নিৰীক্ষণ",
      safetyInsights: "সুৰক্ষা আৰু তথ্য",
      report: "বিপদৰ প্ৰতিবেদন",
      more: "অধিক",
      sos: "জৰুৰীকালীন SOS",
      fullScreen: "মানচিত্ৰ বহলাওক",
      exitFullScreen: "সাধাৰণ দৃশ্য",
      layers: "জিআইএছ স্তৰসমূহ",
      legend: "বিপদৰ চিন",
      recenter: "মোৰ স্থান"
    },
    moreSection: {
      title: "সুৰক্ষা তথ্য আৰু ছেটিংছ",
      subtitle: "দুৰ্যোগ প্ৰস্তুতি নিৰ্দেশনা, জৰুৰীকালীন নম্বৰ আৰু ভাষা ব্যৱস্থা",
      tabGuide: "সুৰক্ষা সহায়িকা",
      tabHelplines: "হেল্পলাইন নম্বৰ",
      tabSettings: "ভাষা আৰু ছেটিংছ",
      tabAbout: "প্ৰণালীৰ বিষয়ে",
      helplineSub: "২৪x৭ ৰাষ্ট্ৰীয় আৰু ৰাজ্যিক জৰুৰীকালীন নম্বৰ",
      openSos: "জৰুৰী SOS খোলক",
      interfaceLanguage: "ইণ্টাৰফেচ ভাষা",
      systemPermissions: "ছিষ্টেমৰ অনুমতিসমূহ",
      locationAccess: "স্থানৰ অনুমতি",
      sirenNotify: "ছাইৰেন জৰুৰী সতৰ্কবাণী",
      relaunchOnboarding: "অনবৰ্ডিং পুনৰ আৰম্ভ কৰক",
      active: "সক্ৰিয়",
      simulated: "আঞ্চলিক মানচিত্ৰ",
      aboutTitle: "লেণ্ড-এলাৰ্ট প্ৰণালীৰ বিষয়ে",
      aboutDesc: "লেণ্ড-এলাৰ্ট হৈছে উত্তৰ-পূব ভাৰতৰ বাবে চেন্সৰ আৰু কৃত্ৰিম বুদ্ধিমত্তা আধাৰিত দুৰ্যোগ ব্যৱস্থাপনা ব্যৱস্থা।"
    },
    onboarding: {
      step1Title: "ভাষা বাছক",
      step1Desc: "আপোনাৰ সুবিধাৰ বাবে ভাষা বাছনি কৰক",
      step2Title: "চৰ্ত আৰু সুৰক্ষা নিৰ্দেশনা",
      step2Desc: "অনুগ্ৰহ কৰি দুৰ্যোগ ব্যৱস্থাপনাৰ নীতি নিয়মসমূহ পঢ়ক",
      step2Checkbox: "মই চৰ্ত আৰু নিৰ্দেশনা মানি লৈছো",
      step3Title: "স্থান আৰু সতৰ্কবাণীৰ অনুমতি",
      step3Desc: "আপোনাৰ ওচৰৰ বিপদ চিনাক্ত কৰিবলৈ জিপিএছ অনুমতি প্ৰয়োজন।",
      allowLocation: "স্থানৰ অনুমতি দিয়ক",
      allowNotify: "সতৰ্কবাণীৰ অনুমতি দিয়ক",
      proceed: "গ্ৰহণ কৰক আৰু আগবাঢ়ক"
    },
    risk: {
      low: "নিম্ন / সুৰক্ষিত",
      moderate: "মধ্যম আশংকা",
      high: "উচ্চ আশংকা",
      critical: "চৰম সংকটপূর্ণ",
      rainfall: "বৰষুণৰ পৰিমাণ",
      soilMoisture: "মাটিৰ আৰ্দ্ৰতা",
      groundMovement: "ভূ-কম্পন",
      slope: "ঢালৰ মাত্ৰা",
      terrain: "পাহাৰীয়া গঠন",
      predictedProb: "ভূমিস্খলনৰ সম্ভাৱনা"
    }
  },
  bn: {
    systemTitle: "ভূমিধস পূর্ব সতর্কবার্তা ও দুর্যোগ ব্যবস্থাপনা সিস্টেম",
    shortTitle: "ল্যান্ড-অ্যালার্ট ইন্ডিয়া",
    amISafe: "আমি কি নিরাপদ?",
    safeStatus: "বর্তমান এলাকা ঝুঁকি: কম",
    unsafeStatus: "বর্তমান এলাকা ঝুঁকি: বৃদ্ধিপ্রাপ্ত",
    criticalStatus: "মারাত্মক ঝুঁকি: ভূমিধসের সম্ভাবনা",
    portals: {
      citizen: "বাসিন্দা পোর্টাল",
      authority: "কর্তৃপক্ষ পোর্টাল",
      fieldOfficer: "মাঠ কর্মকর্তা পোর্টাল",
      admin: "অ্যাডমিন পোর্টাল"
    },
    nav: {
      home: "হোম",
      alerts: "সতর্কবার্তা",
      monitoring: "ঝুঁকি পর্যবেক্ষণ",
      safetyInsights: "নিরাপত্তা ও অন্তর্দৃষ্টি",
      report: "রিপোর্ট",
      more: "আরও",
      sos: "জরুরি এসওএস"
    },
    moreSection: {
      title: "নিরাপত্তা তথ্য ও সেটিংস",
      subtitle: "দুর্যোগ প্রস্তুতি গাইড, জরুরি ডিরেক্টরি এবং ভাষা কনফিগারেশন",
      tabGuide: "সারভাইভাল গাইড",
      tabHelplines: "হেল্পলাইন নম্বর",
      tabSettings: "ভাষা ও সেটিংস",
      tabAbout: "সিস্টেম সম্পর্কে",
      helplineSub: "যাচাইকৃত ২৪x৭ জাতীয় ও রাজ্য জরুরি হটলাইন",
      openSos: "জরুরি SOS খুলুন",
      interfaceLanguage: "ইন্টারফেসের ভাষা",
      systemPermissions: "সিস্টেম অনুমতিসমূহ",
      locationAccess: "অবস্থান অ্যাক্সেস",
      sirenNotify: "সাইরেন জরুরি বিজ্ঞপ্তি",
      relaunchOnboarding: "অনবোর্ডিং পুনরায় শুরু করুন",
      active: "সক্রিয়",
      simulated: "আঞ্চলিক অবস্থান",
      aboutTitle: "ল্যান্ড-অ্যালার্ট সিস্টেম সম্পর্কে",
      aboutDesc: "ল্যান্ড-অ্যালার্ট উত্তর-পূর্ব ভারতের জন্য সেন্সর এবং এআই সমন্বিত অত্যাধুনিক দুর্যোগ পূর্বাভাস প্ল্যাটফর্ম।"
    },
    onboarding: {
      step1Title: "ভাষা নির্বাচন করুন",
      step1Desc: "সতর্কবার্তা পেতে ভাষা নির্বাচন করুন",
      step2Title: "শর্তাবলী ও সম্মতি",
      step2Checkbox: "আমি শর্তাবলী মেনে চলছি",
      step3Title: "লোকেশন ও নোটিফিকেশন",
      proceed: "সম্মতি জানিয়ে এগিয়ে যান"
    },
    risk: {
      low: "কম ঝুঁকি",
      moderate: "মাঝারি ঝুঁকি",
      high: "উচ্চ ঝুঁকি",
      critical: "চরম বিপদ"
    }
  },
  mn: {
    systemTitle: "লৈচিং তারকপগী অঙনবা ৱার্নিং অমসুং অৱা-অনা য়েংশিনবা সিস্তেম",
    shortTitle: "লেন্দ-অলর্ট মনিপুর",
    amISafe: "ঐহাক শেফ ওইব্রা?",
    portals: {
      citizen: "বাসিন্দা পোর্তেল",
      authority: "অথোরিতিগী পোর্তেল",
      fieldOfficer: "ফিল্ড ওফিসার পোর্তেল",
      admin: "এদমিন পোর্তেল"
    },
    nav: {
      home: "য়ুম",
      alerts: "ৱার্নিং",
      monitoring: "Risk Monitoring",
      safetyInsights: "Safety & Insights",
      report: "রিপোর্ত তৌবা",
      more: "অতোপ্পা",
      sos: "জরুরি SOS"
    },
    moreSection: {
      title: "Safety Information & Settings",
      subtitle: "Disaster preparedness guides, emergency directory, and language configuration",
      tabGuide: "Survival Guide",
      tabHelplines: "Helpline Numbers",
      tabSettings: "Language & Settings",
      tabAbout: "About System",
      helplineSub: "Verified 24x7 Emergency Hotlines",
      openSos: "Open SOS",
      interfaceLanguage: "Interface Language",
      systemPermissions: "System Permissions",
      locationAccess: "Location Access",
      sirenNotify: "Siren Notifications",
      relaunchOnboarding: "Re-launch Onboarding Setup",
      active: "ACTIVE",
      simulated: "SIMULATED",
      aboutTitle: "About LandAlert System",
      aboutDesc: "LandAlert is a predictive early warning disaster management system for North-East India."
    },
    risk: {
      low: "চিংনবা লৈতে",
      moderate: "মচং ওইবা",
      high: "য়াম্না অরুবা",
      critical: "অকিবিকা থোকপা"
    }
  },
  ne: {
    systemTitle: "पहिरो पूर्व चेतावनी तथा विपद् व्यवस्थापन प्रणाली",
    shortTitle: "ल्याण्ड-अलर्ट",
    amISafe: "के म सुरक्षित छु?",
    portals: {
      citizen: "निवासी पोर्टल",
      authority: "प्राधिकरण पोर्टल",
      fieldOfficer: "फिल्ड अफिसर पोर्टल",
      admin: "एडमिन पोर्टल"
    },
    nav: {
      home: "गृह",
      alerts: "चेतावनी",
      monitoring: "जोखिम अनुगमन",
      safetyInsights: "सुरक्षा र अन्तर्दृष्टि",
      report: "जोखिम रिपोर्ट",
      more: "थप जानकारी",
      sos: "आपतकालीन SOS"
    },
    moreSection: {
      title: "सुरक्षा जानकारी र सेटिङहरू",
      subtitle: "विपद् पूर्वतयारी मार्गनिर्देशन, आपतकालीन निर्देशिका र भाषा सेटिङ",
      tabGuide: "सुरक्षा गाइड",
      tabHelplines: "हेल्पलाइन नम्बरहरू",
      tabSettings: "भाषा र सेटिङहरू",
      tabAbout: "प्रणालीको बारेमा",
      helplineSub: "प्रमाणित २४x७ आपतकालीन हटलाइनहरू",
      openSos: "द्रुत SOS खोल्नुहोस्",
      interfaceLanguage: "इन्टरफेस भाषा",
      systemPermissions: "प्रणाली अनुमतिहरू",
      locationAccess: "स्थान पहुँच",
      sirenNotify: "साइरन आपतकालीन सूचनाहरू",
      relaunchOnboarding: "अनबोर्डिङ पुनः सुरु गर्नुहोस्",
      active: "सक्रिय",
      simulated: "सिमुलेटेड क्षेत्रीय",
      aboutTitle: "ल्याण्ड-अलर्ट प्रणालीको बारेमा",
      aboutDesc: "ल्याण्ड-अलर्ट उत्तर-पूर्वी भारतका लागि अत्याधुनिक सेन्सर, एआई मोडेल र जीआईएस प्रविधिमा आधारित विपद् व्यवस्थापन प्रणाली हो।"
    },
    risk: {
      low: "सुरक्षित",
      moderate: "मध्यम",
      high: "उच्च जोखिम",
      critical: "अति संवेदनशील"
    }
  },
  te: {
    systemTitle: "కొండచరియల ముందస్తు హెచ్చరిక & విపత్తు నిర్వహణ వ్యవస్థ",
    shortTitle: "ల్యాండ్‌అలర్ట్ ఇండియా",
    amISafe: "నేను సురక్షితంగా ఉన్నానా?",
    safeStatus: "ప్రస్తుత ప్రాంత ప్రమాదం: తక్కువ",
    unsafeStatus: "ప్రస్తుత ప్రాంత ప్రమాదం: పెరిగింది",
    criticalStatus: "తీవ్రమైన ప్రమాదం: కొండచరియలు విరిగిపడే అవకాశం ఎక్కువ",
    portals: {
      citizen: "నివాసితుల పోర్టల్",
      authority: "అధికారుల పోర్టల్",
      fieldOfficer: "ఫీల్డ్ ఆఫీసర్ పోర్టల్",
      admin: "అడ్మిన్ పోర్టల్"
    },
    nav: {
      home: "హోమ్",
      alerts: "హెచ్చరికలు",
      monitoring: "ప్రమాద పర్యవేక్షణ",
      safetyInsights: "భద్రత & అంతర్దృష్టులు",
      report: "రిపోర్ట్",
      more: "మరిన్ని వివరాలు",
      sos: "అత్యవసర SOS",
      fullScreen: "మ్యాప్ విస్తరించు",
      exitFullScreen: "పూర్తి స్క్రీన్ నుండి నిష్క్రమించు",
      layers: "GIS పొరలు",
      legend: "ప్రమాద సూచిక",
      recenter: "నా స్థానం"
    },
    moreSection: {
      title: "భద్రతా సమాచారం & సెట్టింగ్‌లు",
      subtitle: "విపత్తు సంసిద్ధత మార్గదర్శకాలు, అత్యవసర డైరెక్టరీ మరియు భాషా సెట్టింగ్‌లు",
      tabGuide: "రక్షణ గైడ్",
      tabHelplines: "హెల్ప్‌లైన్ నంబర్లు",
      tabSettings: "భాష & సెట్టింగ్‌లు",
      tabAbout: "వ్యవస్థ గురించి",
      helplineSub: "ధృవీకరించబడిన 24x7 జాతీయ మరియు రాష్ట్ర అత్యవసర హాట్‌లైన్‌లు",
      openSos: "త్వరిత SOS ఓపెన్ చేయండి",
      interfaceLanguage: "యాప్ భాష",
      systemPermissions: "సిస్టమ్ అనుమతులు",
      locationAccess: "లొకేషన్ యాక్సెస్",
      sirenNotify: "సైరన్ అత్యవసర నోటిఫికేషన్‌లు",
      relaunchOnboarding: "ఆన్‌బోర్డింగ్‌ను మళ్లీ ప్రారంభించండి",
      active: "క్రియాశీలం",
      simulated: "ప్రాంతీయ అనుకరణ",
      aboutTitle: "ల్యాండ్‌అలర్ట్ వ్యవస్థ గురించి",
      aboutDesc: "ల్యాండ్‌అలర్ట్ అనేది ఈశాన్య మరియు కొండ ప్రాంతాల కోసం అత్యాధునిక IoT సెన్సార్లు, AI నమూనాలు మరియు రియల్-టైమ్ GIS మ్యాపింగ్‌తో కూడిన ప్రభుత్వ స్థాయి విపత్తు ముందస్తు హెచ్చరిక వ్యవస్థ."
    },
    onboarding: {
      title: "ల్యాండ్‌అలర్ట్‌కు స్వాగతం",
      subtitle: "ప్రభుత్వ స్థాయి కొండచరియల ముందస్తు హెచ్చరిక మరియు విపత్తు నిర్వహణ వేదిక",
      step1Title: "మీ భాషను ఎంచుకోండి",
      step1Desc: "భద్రతా హెచ్చరికలు మరియు సూచనల కోసం మీకు నచ్చిన భాషను ఎంచుకోండి",
      step2Title: "నిబంధనలు & భద్రతా ఒప్పందం",
      step2Desc: "దయచేసి అధికారిక విపత్తు డేటా వినియోగం మరియు నివాసితుల మార్గదర్శకాలను సమీక్షించండి",
      step2Terms: "కొనసాగడం ద్వారా, ల్యాండ్‌అలర్ట్ అందించే AI-ఆధారిత అంచనాలు మరియు అత్యవసర సలహాలకు మీరు అంగీకరిస్తున్నారు. విపత్తు సమయాల్లో ఎల్లప్పుడూ అధికారిక NDRF, SDRF మరియు జిల్లా యంత్రాంగం తరలింపు ఆదేశాలను పాటించండి.",
      step2Checkbox: "నేను నిబంధనలు, షరతులు మరియు అత్యవసర మార్గదర్శకాలను అంగీకరిస్తున్నాను",
      step3Title: "అవసరమైన అనుమతులు",
      step3Desc: "స్థానికీకరించిన ప్రాణరక్షణ హెచ్చరికల కోసం లొకేషన్ మరియు నోటిఫికేషన్ అనుమతులు తప్పనిసరి.",
      locationTitle: "లొకేషన్ అనుమతి",
      locationDesc: "నిజ-సమయ ప్రమాద మండలాలకు మీ స్థానాన్ని గుర్తించడానికి మరియు స్థానిక వాలు స్థిరత్వాన్ని లెక్కించడానికి ఉపయోగించబడుతుంది.",
      notifyTitle: "అత్యవసర నోటిఫికేషన్ అనుమతి",
      notifyDesc: "కొండచరియలు విరిగిపడటం మరియు ఆకస్మిక వరదల కోసం తక్షణ సైరన్ హెచ్చరికలను అందించడానికి ఉపయోగించబడుతుంది.",
      allowLocation: "లొకేషన్ యాక్సెస్ అనుమతించు",
      allowNotify: "సైరన్ నోటిఫికేషన్‌లను అనుమతించు",
      granted: "అనుమతించబడింది ✓",
      proceed: "అంగీకరించి కొనసాగండి",
      tryAgain: "మళ్లీ ప్రయత్నించండి",
      continueWithoutLocation: "ప్రాంతీయ మ్యాప్‌తో కొనసాగండి",
      disclaimer: "వ్యక్తిగతీకరించిన ప్రమాద సమాచారం కోసం లొకేషన్ యాక్సెస్ అవసరం. తిరస్కరించినట్లయితే, సాధారణ ప్రాంతీయ మ్యాప్ చూపబడుతుంది."
    },
    risk: {
      low: "తక్కువ / సురక్షితం",
      moderate: "మధ్యస్థ ప్రమాదం",
      high: "అధిక ప్రమాదం",
      critical: "తీవ్రమైన ప్రమాదం",
      rainfall: "24గం వర్షపాతం",
      soilMoisture: "నేల తేమ",
      groundMovement: "భూమి కంపనం",
      slope: "వాలు కోణం",
      terrain: "భౌగోళిక భూభాగం",
      predictedProb: "అంచనా వేసిన ప్రమాద సంభావ్యత",
      whyRiskHigh: "ప్రమాదం ఎందుకు ఎక్కువగా ఉంది?",
      safetyPrecautions: "భద్రతా జాగ్రత్తలు"
    },
    report: {
      title: "ప్రజల ప్రమాద నివేదిక",
      subtitle: "క్షేత్ర పరిశీలనలను నేరుగా జిల్లా అత్యవసర ఆపరేషన్ కేంద్రానికి సమర్పించండి",
      hazardType: "ప్రమాద రకాన్ని ఎంచుకోండి",
      description: "ప్రమాదం / పగుళ్లు / శిథిలాల వివరాలు",
      photoProof: "ఫోటో / వీడియో జోడించండి",
      captureGps: "ప్రస్తుత GPS గుర్తించండి",
      submitBtn: "అధికారులకు నివేదిక సమర్పించండి",
      successTitle: "నివేదిక విజయవంతంగా సమర్పించబడింది",
      successDesc: "మీ సంఘటన నివేదిక రాష్ట్ర విపత్తు నిర్వహణ అధికార యంత్రాంగానికి పంపబడింది మరియు ట్రాకింగ్ ఐడీ కేటాయించబడింది.",
      reportId: "రిపోర్ట్ ఐడీ"
    },
    guide: {
      title: "కొండచరియల రక్షణ గైడ్",
      before: "కొండచరియలు విరిగిపడక ముందు",
      during: "కొండచరియలు విరిగిపడుతున్నప్పుడు",
      after: "కొండచరియలు విరిగిపడిన తర్వాత",
      kit: "అత్యవసర కిట్ వస్తువులు"
    }
  }
};
