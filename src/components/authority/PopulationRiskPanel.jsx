  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div className="glass-panel" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '20px' }}>
        <div style={{ width: '56px', height: '56px', borderRadius: '14px', background: 'rgba(211, 47, 47, 0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {locations.map(loc => (
          <div key={loc.id} className="glass-panel" style={{ padding: '18px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              <div>
                <h4 style={{ color: '#fff', fontSize: '1.0rem' }}>{loc.name}</h4>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
              ...
              <span style={{ fontSize: '0.88rem', color: 'var(--text-secondary)' }}>
                Population at Hazard: <strong style={{ color: '#fff' }}>{(loc.populationAtRisk || 0).toLocaleString()}</strong>
              </span>
            </div>
            ...
            {loc.shelters && loc.shelters.length > 0 && (
              <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: '10px' }}>
                ...
                {loc.shelters.map((sh, idx) => (
                  <div key={idx} style={{ background: 'rgba(7, 21, 34, 0.6)', padding: '8px 10px', borderRadius: '6px', marginBottom: '6px', fontSize: '0.78rem' }}>
                    <div style={{ fontWeight: 600, color: '#fff' }}>{sh.name}</div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)', marginTop: '2px' }}>
                      ...
                      <span style={{ color: '#00E676' }}>Occupied: {sh.occupied}</span>
                    </div>
                  </div>
                ))}
              </div>
