...
const SensorManagement = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
      {/* AUTOMATED SENSOR FAILURE NOTIFICATION BANNER */}
        ...
        <div key={alert.id} className="sensor-failure-alert-banner">
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            ...
            <button 
              ...
              style={{ padding: '8px 14px', fontSize: '0.82rem', borderColor: '#4CAF50', color: '#2E7D32' }}
      ...
      <div className="data-table-container">
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3 style={{ fontSize: '1.1rem', color: 'var(--color-navy)', fontWeight: 700, marginBottom: '2px' }}>
          ...
          <button className="btn-secondary" style={{ fontSize: '0.8rem', padding: '6px 12px' }} onClick={() => setIsFullScreenMap(true)}>
        ...
        <table className="data-table">
          ...
          <tbody>
            {sensors.map(sensor => {
              ...
              return (
                <tr key={sensor.id}>
                  <td style={{ fontWeight: 800, color: 'var(--color-blue-600)', fontFamily: 'var(--font-mono)' }}>
                    {sensor.id}
                  </td>
                  <td>
                    <div style={{ fontWeight: 600, color: 'var(--color-navy)' }}>{sensor.name}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>{sensor.typeLabel}</div>
                  </td>
      ...
      {selectedSensor && (
        <div className="modal-overlay" onClick={() => setSelectedSensor(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ padding: '24px', maxWidth: '520px' }}>
            ...
            <div style={{ background: 'var(--color-blue-50)', border: '1px solid var(--color-border)', padding: '14px', borderRadius: '10px', marginBottom: '14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem' }}>
                <span>Telemetry Parameter:</span>
                <strong style={{ color: 'var(--color-navy)' }}>{selectedSensor.typeLabel}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem' }}>
                <span>Current Real-time Reading:</span>
                <strong style={{ color: 'var(--color-blue-600)', fontSize: '1.1rem' }}>{selectedSensor.value} {selectedSensor.unit}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '0.85rem' }}>
                <span>Lithium Battery Bank:</span>
                <strong style={{ color: 'var(--color-risk-low)' }}>{selectedSensor.battery}% Health</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                <span>LoRa Uplink Signal:</span>
                <strong style={{ color: 'var(--color-navy)' }}>{selectedSensor.signal} dBm (Good)</strong>
              </div>
            </div>
